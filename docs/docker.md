...
2. `NestJS` tells `Docker` to spin up the container, but instead of hooking into text streams, it maps an internal port to an external one (e.g., 8080 to 8081).
3. `NestJS` waits a few seconds for the virtual monitor (`Xvfb`) to boot up.
4. `NestJS` replies to the frontend: "Your container is ready. Connect your `noVNC` canvas directly to port 8081."
5. From that point on, the frontend handles the heavy video streaming directly via raw `WebSockets`, leaving our backend free from the heavy lifting.
